...
- [ ] 1.1 Add a `Config` struct and loader that parses `PORT`,
      ...
      variable.
- [ ] 1.2 Add a test asserting the `Config` struct's field set matches the
...
- [ ] 2.1 Add a `pgxpool`-based connection pool built from
      `Config.DatabaseURL`; verify with a test (against the CI/local
      Postgres) that the pool can ping the database.
- [ ] 2.2 Add an embedded (`go:embed`) migrations directory wired to
...
- [ ] 2.3 Run migrations automatically at process startup, before the HTTP
...
- [ ] 3.1 Implement `GET /api/healthz` per `specs/platform/healthcheck`:
      ...
      handler test.
- [ ] 3.2 Wire the stdlib `ServeMux` so every path under `/api/` is
...
- [ ] 3.3 Embed `openapi.yaml` and serve it at `GET /api/openapi.yaml`;
      verify with a test comparing the response body byte-for-byte to the
      embedded file.
- [ ] 3.4 Embed `static/out/` and serve it for `GET` requests outside
...
- [ ] 4.1 Add subcommand dispatch in `main()`: `healthcheck` as the first
      ...
      logic.
- [ ] 4.2 Implement the healthcheck probe: `GET
...
- [ ] 5.1 Add a `postgres:17` service to `.github/workflows/ci.yml`'s
...
- [ ] 5.2 Run `docker compose up --build` locally and confirm the `app`
      ...
      retries; record the result.

## 6. Verify the change

- [ ] 6.1 Run `go build ./...`, `go vet ./...` and `go test ./...` in
      `backend/` (against a local Postgres via `docker compose up -d db`)
      and confirm all pass.
- [ ] 6.2 Run `openspec validate backend-foundation` and confirm it passes,
      including both new capabilities.
- [ ] 6.3 Confirm `git status` shows no changes under `frontend/`,
