...
- [x] 1.1 Create `docs/parity-checklist.md` with the four sections (Screens,
...
- [x] 1.2 Write the id conventions into the header — `SCR-`, `EP-`, `DRV-`,
...
- [x] 2.1 List every screen under `old/pages/` that is not an API route as one
...
- [x] 2.2 List every handler under `old/pages/api/v1/` as one `EP-` line each,
...
- [x] 2.3 Record the derived values as `DRV-` lines with their exact formulas
      ...
      character.
- [x] 2.4 Restate the Scope decisions table as `NG-` lines (password login,
...
- [x] 3.1 Add a "Flagged for `api-contract-v1`" subsection recording the three
...
- [x] 4.1 Add a "Parity" section to `AGENTS.md` stating that
...
- [x] 4.2 Link the checklist from `README.md` in the Workflow section, next to
...
- [x] 5.4 Confirm `git status` shows no modification under `old/`; the legacy
