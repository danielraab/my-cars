...
The frontend SHALL provide a login route that accepts an email for magic-link
delivery and offers OIDC login. It SHALL send magic-link requests to the
documented backend operation and initiate OIDC with a full-page navigation to
the documented backend route; it SHALL NOT contact an identity provider
directly or request, receive, or persist an authentication token.
